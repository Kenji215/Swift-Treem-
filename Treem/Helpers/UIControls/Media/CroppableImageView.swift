...
//  Treem
//
//  Created by Tracy Merrill on 2/1/16.
//  Copyright © 2016 Treem LLC. All rights reserved.
//
// When using this, you must set the assigned view as "Clear" background.
...
class CroppableImageView: UIView, CornerpointClientProtocol
{
    ...
    var  imageToCrop: UIImage?
        ...
        didSet
        {
            ...
            viewForImage.image = imageToCrop
            self.setNeedsLayout()
        }
    }
    
    let viewForImage: UIImageView
    ...
    private var imageCenter : CGFloat?
    ...
    required init(coder aDecoder: NSCoder)
    {
        ...
        viewForImage = UIImageView(frame: CGRectZero)
    ...
    override func layoutSubviews()
    {
        ...
        if let requiredImageSize = imageSize
        {
            ...
            displaySize.width = min(requiredImageSize.width, self.frame.width)
            displaySize.height = min(requiredImageSize.height, self.frame.height)
            let heightAspect: CGFloat = displaySize.height/requiredImageSize.height
            let widthAspect: CGFloat = displaySize.width/requiredImageSize.width
            aspect = min(heightAspect, widthAspect)
            
            if (requiredImageSize.width > requiredImageSize.height) {
                displaySize.height = round(requiredImageSize.height * aspect)
                displaySize.width = round(requiredImageSize.width * aspect)
                
                imageCenter = (self.frame.height - displaySize.height) / 2
                
                imageRect = CGRectMake(0, imageCenter!, displaySize.width, displaySize.height)
            }
            else {
                displaySize.height = round(requiredImageSize.height * heightAspect)
                displaySize.width = round(requiredImageSize.width * widthAspect)
                
                imageRect = CGRectMake(0, 0, displaySize.width, displaySize.height)
            }
            
            imageViewSize = imageRect!.size
        }
    ...
    */
    func croppedImage() -> UIImage?
    {
        if var cropRect = internalCropRect
        {
            ...
            drawRect.origin.y = round(-cropRect.origin.y / ((aspect * imageSize!.height)) - (imageCenter! * 2))
